...
def _index_tags(category, tags):
    ...
    result = dict(dict(zip(tags, range(TOTAL_IDX, TOTAL_IDX + len(tags)))))
    TOTAL_IDX += len(tags)
...
FORMANT_IDX = _index_tags('formant', [
    'f1:centre.hz', 'f1:bandwidth.hz', 'f2:centre.hz', 'f2:bandwidth.hz',
...
def get_formants(info, start, end):
    ...
    for formants in info['formants']:
        ...
        if formant_time >= start and formant_time < end:
            if 'F1' in formants:
                f1.append((formants['F1']['centre.hz'],
                           formants['F1']['bandwidth.hz']))
            if 'F2' in formants:
                f2.append((formants['F2']['centre.hz'],
                           formants['F2']['bandwidth.hz']))
            if 'F3' in formants:
                f3.append((formants['F3']['centre.hz'],
                           formants['F3']['bandwidth.hz']))
...
def _serialize_example(key, label, info):
    ...
    for i, start in enumerate(np.arange(0, end, 0.05)):
        interval_indexes, interval_weights = _interval_string(
            label, info, start, start + 0.05)
...
class LabelRecordGenerator:
    ...
    def generate_result(self, audio_archive, info_archive):
        def load_by_key(clip_id):
            clip_id = clip_id.experimental_ref().deref().numpy().decode(
                'utf-8')
        ...
        def tf_load_by_key(clip_id):
            [
                example,
            ] = tf.py_function(load_by_key, [clip_id], [tf.string])
            return example
        ...
        dataset = dataset.apply(
            tf.data.experimental.group_by_window(lambda i, _: i % n_shards,
