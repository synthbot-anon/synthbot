...
class AudioTarGenerator:
    ...
    def get_character_tar(self, character):
        ...
        character_tar_path = os.path.join(
            self.output_dir, '{}.tar'.format(normalized_character))
    ...
    def generate_result(self, reference: str, audio: clipper_in.ClipperFile,
                        alignments: mfa_in.MFAAlignmentsFile):
        ...
        output_audio_path = os.path.join(reference,
        ...
        label = {
            ...
            'utterance': {
                'content':
                audio.label['transcript'],
                'episode_interval':
                [float(audio.label['start']),
                 float(audio.label['end'])]
            },
