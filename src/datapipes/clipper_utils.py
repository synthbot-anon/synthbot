...
def load_ponysorter_data(context, path, label_data):
    ...
    tags = [context.tags(x) for x in label_data['mood']]

    noise_key = label_data['noise_level']
    noise = context.noise_levels(noise_key)
...
def load_audacity_data(context, line_data):
    ...
    if label_parts[4] == 'Canterlot Voice':
        tags = context.tags('Canterlot Voice')
    else:
        tags = [context.tags(x) for x in label_parts[4].split()]

    # noise level
    noise_level = context.noise_levels(label_parts[5])
