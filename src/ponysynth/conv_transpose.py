# Taken from Sonnet 2.0 with minor modifications.
...
from ponysynth import sonnet_types as types
...
class ConvNDTranspose(base.Module):
  ...
  """
    ...
    def __call__(self, inputs):
        ...
        outputs = tf.nn.conv_transpose(input=inputs,
                                       filters=self.w,
    ...
    def _initialize(self, inputs):
        ...
        self.w = self._make_w()
        if self._with_bias:
            self.b = tf.Variable(self._b_init((self._output_channels, ),
                                              self._dtype),
                                 name="b")

    def _make_w(self):
        """Makes and returns the variable representing the weight."""
        weight_shape = utils.replicate(self._kernel_shape,
                                       self._num_spatial_dims, "kernel_shape")
        weight_shape = weight_shape + (self._output_channels,
                                       self.input_channels)
    ...
    def _get_window_shape(self, input_shape):
        if self._channel_index == 1:
            input_size = input_shape[2:]
        else:
            input_size = input_shape[1:-1]
        stride = utils.replicate(self._stride, self._num_spatial_dims,
                                 "stride")

        window_shape = [x * y for (x, y) in zip(input_size, stride)]

        if self._padding == "VALID":
            kernel_shape = utils.replicate(self._kernel_shape,
                                           self._num_spatial_dims,
                                           "kernel_shape")
            rate = utils.replicate(self._rate, self._num_spatial_dims, "rate")
            effective_kernel_shape = [(shape - 1) * rate + 1
                                      for (shape,
                                           rate) in zip(kernel_shape, rate)]
            window_shape = [
                x + y - 1
                for (x, y) in zip(window_shape, effective_kernel_shape)
            ]
    ...
    def _get_output_shape(self, inputs):
        ...
        if output_shape is None:
            output_shape = self._get_window_shape(inputs.shape)
        elif len(output_shape) != self._num_spatial_dims:
            raise ValueError(
                "The output_shape must be of length {} but instead was {}".
                format(self._n, len(output_shape)))

        if self._channel_index == 1:
            output_shape = (self._output_channels, ) + output_shape
        else:
            output_shape = output_shape + (self._output_channels, )

        output_shape = tf.concat([[tf.shape(inputs)[0]], output_shape], 0)
